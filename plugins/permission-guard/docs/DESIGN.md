...
1. **パス包含チェック**: `python3 scripts/analyze.py` → allow（プロジェクト内）/ `python3 /tmp/evil.py` → ask（外部）
2. **インタプリタ+フラグ判定**: `bash scripts/deploy.sh` → allow / `bash -c "arbitrary code"` → ask
3. **サブコマンド粒度**: `git status` → allow / `git push` → ask
4. **危険フラグ検出**: `python3 -u script.py` → allow / `python3 -c "code"` → ask
...
- Phase 6: `scripts/` or `.claude/hooks/` auto-approval
- Phase 7: general command（always_ask, subcommand_ask, path containment）
