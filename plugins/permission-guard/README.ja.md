...
すべてのコマンドは2つのステージを通過します。**pre-validation**（入力サニタイズ）と **AST ベース検証**（tree-sitter-bash による構造解析）です。
...
### AST パース（tree-sitter-bash）

Pre-validation の後、コマンドを [tree-sitter-bash](https://github.com/tree-sitter/tree-sitter-bash) で AST にパースします。ヒアドキュメント、クォート文字列、特殊変数を正しく扱える構造解析を提供します。

AST ウォーカーがノード種別で危険な構文を検出します：

| AST ノード | 検出する構文 | 判定 |
|----------|-------------------|----------|
| `command_substitution` | `` `cmd` `` または `$(cmd)` | deny |
| `simple_expansion` / `expansion` | `$VAR`, `${VAR}` 等 | deny |
| `variable_assignment` | `FOO=bar` | ask |
| バックグラウンド `&` | バックグラウンド実行 | deny |
| `word` 内の非クォートグロブ文字 | `*`, `?`, `[` | deny |

安全な特殊変数（`$?`, `$#`, `$!`, `$-`, `$0`, `$_`, `$@`, `$$`）は認識され許可されます。クォート文字列（`'...'`, `"..."`）内のグロブ文字はフラグされません。

tree-sitter-bash がコマンドをパースできない場合（または AST にエラーがある場合）、判定は **ask** にフォールバック（安全なデフォルト）。
...
Pre-validation の失敗と危険な AST ノードは **deny**（`variable_assignment` は **ask**）。NEVER_SAFE と未登録コマンドは **ask**。それ以外は tools の設定に従います。
...
- **PyYAML** -- 設定ファイルの読み込み
- **tree-sitter** / **tree-sitter-bash** -- シェルコマンドの AST パース
...
│   │   ├── parser.py          # tree-sitter-bash AST パーサー
